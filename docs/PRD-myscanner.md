...
- **Kontext**: Papírové a digitální doklady je nutné převést na text a klíčové údaje; typ dokumentu má řídit, které *standardní údaje* se zobrazí, a doplnit je o *Kompletní přepis* (celý rozpoznaný text dokumentu).
...
- **In scope**:
  ...
  - Zobrazení *standardních údajů* podle typu a jedné sekce *Kompletní přepis* s celým rozpoznaným textem z dokumentu.
  - *Historie* naskenovaných položek, filtrování podle typu, mazání položky v rámci relace, náhled *Kompletního přepisu* i původního vstupu (obrázek / náhled souboru); u obrázkového vstupu po klepnutí na náhled zobrazení v originálním rozlišení.
...
- R5: Aplikace odešle obsah vstupu (obrázek / extrahovatelná data) do OpenAI API podle schváleného modelu a získá strukturovaný výsledek: navržený typ mezi *faktura*, *účtenka z obchodu*, *vizitka*, textový výstup pro *Kompletní přepis* a vyplnění *standardních údajů* podle navrženého typu.
...
### US-3: Zobrazení výsledku (*standardní údaje* a *Kompletní přepis*)

**User story**: Jako uživatel chci vidět *standardní údaje* a pod nimi sekci *Kompletní přepis* s celým rozpoznaným textem dokumentu, abych měl přehled o strukturovaných polích i o plném textu.
...
- R9: Pod *standardními údaji* se zobrazí sekce *Kompletní přepis* s jedním souvislým textovým výstupem rozpoznání (bez samostatného úvodního „shrnutí“ v UI ani v požadavcích na strukturu výstupu). *Standardní údaje* zůstávají v samostatné sekci nad *Kompletním přepisem* (odlišení strukturovaných polí od volného textu).
- R10: Uložená položka v *historii* obsahuje: časové razítko skenu, aktuální typ (*štítek*), *standardní údaje*, text *Kompletního přepisu*, referenci na vstupní médium pro náhled (v paměti relace).

### US-4: Úprava *štítku* (typu dokumentu)

**User story**: Jako uživatel chci změnit navržený typ dokumentu, když se systém splete, aby *standardní údaje* a *Kompletní přepis* odpovídaly mému úmyslu (v rámci již rozpoznaného textu).

**Requirements**:

- R11: Uživatel může změnit typ na kterýkoli ze tří typů z tabulky; UI nabízí jednoznačnou volbu (picker, segmenty apod.).
- R12: Po změně typu uživatelem aplikace **nesmí znovu volat OpenAI API**; aktualizuje zobrazení *standardních údajů* a obsahu *Kompletního přepisu* výhradně z již získaného výsledku prvního rozpoznání — doplnění polí z již rozpoznaného textu, jinak prázdná pole nebo „—“ podle UX v sekci Specifikace typů.
...
- R15: Z detailu položky v *historii* je dostupný náhled původního vstupu (obrázek / náhled souboru dle typu vstupu) a stejná struktura obrazovky jako po skenu: *standardní údaje* a *Kompletní přepis*. U položky s obrázkovým vstupem po klepnutí na náhled uživatel vidí obrázek v **originálním rozlišení**.
...
- **Kompletní přepis**: Název sekce v UI pod *standardními údaji*; zobrazuje jeden souvislý textový výstup rozpoznání celého dokumentu.
- **Přepis**: Textový výstup rozpoznání dokumentu; v aplikaci prezentovaný v sekci *Kompletní přepis*.
